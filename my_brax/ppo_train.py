...
def _apply_redo(
    ...
    # Get default hidden layer sizes (brax default is (256, 256))
    policy_hidden_sizes = [256, 256]  # Default brax hidden sizes
    ...
    # Keep the optimizer state as-is (the paper suggests this is fine)
    return training_state.replace(params=new_params)
...
def train(
    ...
    redo_tau: float = 0.01,  # Threshold for dormant neuron detection
):
  """PPO training.
    ...
    for _ in range(max(num_resets_per_eval, 1)):
      ...
      if use_redo and (it + 1) % redo_frequency == 0:
        ...
        try:
          ...
          logging.warning(f'Failed to apply ReDo at iteration {it + 1}: {e}')

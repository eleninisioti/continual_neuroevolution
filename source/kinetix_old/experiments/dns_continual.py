...
def create_play_step_fn(env, env_params, network):
    ...
    def play_step_fn(obs, env_state, policy_state, done, key):
        ...
        # Preserve done shape (1,) for scan compatibility
        step_done_shaped = jnp.reshape(step_done, (1,))
        return next_obs, next_env_state, updated_policy_state, step_done_shaped, key, transition
...
def scoring_function_kinetix(
    ...
    # Average fitness across rollouts
    fitnesses = jnp.mean(all_fitnesses, axis=0)

    # For descriptors, select from rollout with median fitness (per individual)
    sorted_indices = jnp.argsort(all_fitnesses, axis=0)  # (num_evals, batch_size)
    median_idx = num_evals // 2
    median_rollout_indices = sorted_indices[median_idx]  # (batch_size,)
    batch_indices = jnp.arange(all_descriptors.shape[1])
    descriptors = all_descriptors[median_rollout_indices, batch_indices]  # (batch_size, desc_dim)
...
def train_dns_continual(
    *,
    ...
    def save_task_gifs(
        ...
        init_env_state, best_policy_state, num_gifs=10, eval_episode_length=None,
    ):
        # Use provided episode_length or fall back to env max_timesteps
        max_steps = eval_episode_length if eval_episode_length is not None else ep_t.max_timesteps
...
def parse_args():
    ...
    parser.add_argument("--num_evals", type=int, default=10,
                        help="Number of rollouts averaged for fitness (default 10)")

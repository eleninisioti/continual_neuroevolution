...
def train_ga_continual(
    *,
    ...
    es_params = es_params.replace(crossover_rate=crossover_rate)

    # Initialize archive with num_elites individuals (matches inspiration)
    num_elites = strategy.num_elites
    print(f"  num_elites (archive size): {num_elites}")
    rng, pop_rng = jr.split(rng)
    init_population = jr.uniform(pop_rng, (num_elites, num_dims), minval=-1.0, maxval=1.0)
    init_fitness = jnp.full(num_elites, jnp.inf)
    ...
    def make_rollout_and_eval(env, ep, init_env_state):
        ...
        def eval_population(flat_pop, rng_key):
            ...
            def scan_fn(carry, batch):
                fit, length = _eval_batch(batch, rep_keys)
                return carry, (fit, length)

            _, (all_fits, all_lens) = jax.lax.scan(scan_fn, None, batched_pop)
            # all_fits: (num_batches, eval_batch_size, total_reps)
            # Reshape back to (popsize, total_reps)
            return all_fits.reshape(popsize, -1), all_lens.reshape(popsize, -1)
    ...
    for task_idx, env_name in enumerate(ENVIRONMENTS):
        ...
        if task_idx > 0:
            n_archive = es_state.population.shape[0]
        ...
        def train_step(carry, gen_idx):
            es_state_, rng_, best_fit_ = carry
            rng_, ask_key, eval_key, tell_key = jr.split(rng_, 4)

            flat_pop, es_state_ = strategy.ask(ask_key, es_state_, es_params)
            ...
            es_state_, _ = strategy.tell(tell_key, flat_pop, -evolve_fit, es_state_, es_params)
        ...
        es_state, rng, task_best_fitness = carry
        dt = time.time() - t0
        ...
        final_fitnesses, _ = eval_pop_fn(final_pop, eval_key)
        final_mean_fit = jnp.mean(final_fitnesses[:, :eval_reps], axis=1)
        best_idx = jnp.argmax(final_mean_fit)
        task_best_params = final_pop[best_idx]
        task_best_fitness = final_mean_fit[best_idx]

    ...
    *,
    ...
    if use_wandb:
        wandb.init(
            ...
            name=f"ES_continual_trial{trial_idx}_seed{seed}",
            config={
                "popsize": popsize,
    ...
    for task_idx, env_name in enumerate(ENVIRONMENTS):
        ...
        for local_gen in range(generations_per_task):
            ...
            # Always use the best from the current generation (es_state.mean)
            task_best_fitness = rep_best_mean
            task_best_mean_params = jnp.array(es_state.mean)

            if rep_best_mean > best_fitness_ever:
                best_fitness_ever = rep_best_mean
                best_mean_params_ever = jnp.array(es_state.mean)

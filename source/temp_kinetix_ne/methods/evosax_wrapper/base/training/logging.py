...
class Logger:
	...
	def log(self, state: TrainState, data: Data, task_params: jnp.array, noise, diversity, current_gravity, 
		...
		# Extract the best individual from the current generation's population
		# state.best_member only updates when fitness improves, not when best_indiv_index changes
		# So we need to extract from data["parameters"] using data["best_indiv"]
		best_member_pytree = jax.tree_map(lambda x: x[data["best_indiv"], ...], data["parameters"])
		# Flatten to match the format that state.best_member uses (for consistency)
		ckpt_data = self.params_shaper.flatten_single(best_member_pytree) if hasattr(self, 'params_shaper') else best_member_pytree
	...
	def save_best_model(self, data: dict, epoch: int):
		...
		if self.ckpt_dir is not None:
			jax.lax.cond(
				jnp.mod(epoch, self.ckpt_freq) == 199,
				lambda data: tap_save(data, epoch),
				lambda data: None,
				data
			)

"""
Train OpenES on CheetahRun with a single friction value (non-continual).

Usage:
    python train_ES_cheetah.py --friction 0.5 --gpus 0
    python train_ES_cheetah.py --friction 1.0 --gpus 0
    python train_ES_cheetah.py --friction 1.5 --gpus 0
"""
...
def main():
    ...
    output_dir = args.output_dir or f"projects/mujoco/es_{env_name}_{friction_label}/trial_{trial}"
    ...
    scoring_fn, rollout_with_trajectory = make_scoring_fn(env, policy, param_template, episode_length)
    ...
    wandb.init(project=args.wandb_project, config=config,
               name=f"es_{env_name}_{friction_label}_trial{trial}", reinit=True)
    ...
    # Save checkpoint
    ckpt_path = os.path.join(output_dir, f"es_{env_name}_{friction_label}_best.pkl")

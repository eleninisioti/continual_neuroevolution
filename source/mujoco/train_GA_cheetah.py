"""
Train GA on CheetahRun with a single friction value (non-continual).

Usage:
    python train_GA_cheetah.py --friction 0.5 --gpus 0
    python train_GA_cheetah.py --friction 1.0 --gpus 0
    python train_GA_cheetah.py --friction 1.5 --gpus 0
"""
...
def make_scoring_fn(env, policy, param_template, episode_length, num_evals=1):
    ...
    def scoring_fn(flat_genotypes, key):
        ...
        if num_evals == 1:
            ...
            # Multiple evaluations per individual - average for stable fitness
...
def main():
    ...
    output_dir = args.output_dir or f"projects/mujoco/ga_{env_name}_{friction_label}/trial_{trial}"
    ...
    ga_state = ga.init(state_init_key, init_population, init_fitness, ga_params)
    ...
    for gen in range(num_generations):
        ...
        population, ga_state = ga.ask(ask_key, ga_state, ga_params)
        ...
        ga_state, _ = ga.tell(tell_key, population, -fitness, ga_state, ga_params)
    ...
    # Save checkpoint
    ckpt_path = os.path.join(output_dir, f"ga_{env_name}_{friction_label}_best.pkl")

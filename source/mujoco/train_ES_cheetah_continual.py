"""
Train OpenES on MuJoCo Playground with Continual Learning (physics changes).
...
Usage:
    python train_openes_continual.py --env HumanoidWalk
    python train_openes_continual.py --env CheetahRun --num_tasks 5 --task_mod gravity
    python train_openes_continual.py --env WalkerRun --task_mod friction --friction_low_mult 0.2 --friction_high_mult 5.0
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='OpenES Continual Learning on MuJoCo Playground')
...
def main():
    ...
    print("=" * 60)
    print(f"OpenES Continual Learning on {env_name} (MuJoCo Playground/MJX)")
    print("=" * 60)
    ...
    if args.output_dir:
        ...
        output_dir = f"results_openes_continual_{env_name.lower()}_{mod_suffix}"
    os.makedirs(output_dir, exist_ok=True)
    
    # Initialize wandb
    run_name = args.run_name if args.run_name else f"openes_continual_{env_name}_{task_mod}_seed{seed}"
    wandb.init(
    ...
    # Initialize Open_ES with tuned hyperparameters
    print("\nInitializing Open_ES...")
    dummy_solution = jnp.zeros(num_params)
    
    # Custom std schedule (constant at sigma)
    std_schedule = lambda gen: sigma
    
    # OpenES uses Adam optimizer internally
    ...
    # Get default params for Open_ES
    es_params = jax.device_put(es.default_params, replicate_sharding)
    
    key, init_key = jax.random.split(key)
    # Initialize ES state (no JIT needed - runs once)
    ...
    print(f"  Learning rate: {learning_rate}")
    print(f"  Sigma (noise std): {sigma}")
    print(f"  Antithetic sampling: True")
    ...
    for task_idx in range(num_tasks):
        ...
        with open(ckpt_path, 'wb') as f:
            pickle.dump({
                ...
                'task_mod': task_mod,
                'multiplier': current_multiplier,
                'task_best_fitness': float(best_fitness_task),
                'overall_best_fitness': float(best_fitness_overall),
                'total_gen': total_gen,
                'env_name': env_name,
            }, f)
        ...
        with open(save_path, 'wb') as f:
            pickle.dump({
                ...
                'config': {
                    ...
                    'num_tasks': num_tasks,
                    'task_mod': task_mod,
                    'task_multipliers': list(multiplier_list),
                    'continual': True,
                }
            }, f)

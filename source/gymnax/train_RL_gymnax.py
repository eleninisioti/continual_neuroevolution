...
def evaluate(key, policy_network, policy_params, env, env_params, num_episodes=10, max_steps=500):
...
def main():
    ...
    @jax.jit
    def jit_collect_rollout_fn(key, policy_params, value_params):
    ...
    for update in range(num_updates):
        ...
        rollout = jit_collect_rollout_fn(
            rollout_key, policy_state.params, value_state.params
        )
        ...
        if (update + 1) % args.eval_interval == 0 or update == num_updates - 1:
            ...
            mean_reward, std_reward = evaluate(
                ...
                env, env_params, args.num_eval_episodes, hp['episode_length']
            )
    ...
    for eval_trial in range(10):
        ...
        for step in range(hp['episode_length']):
            logits = policy_network.apply(policy_state.params, obs)

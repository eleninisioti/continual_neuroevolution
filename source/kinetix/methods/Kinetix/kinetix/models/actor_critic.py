...
class GeneralActorOnlyRNN(nn.Module):
    """Actor-only version of GeneralActorCriticRNN — no critic network."""
    action_dim: Sequence[int]
    fc_layer_depth: int
    fc_layer_width: int
    action_type: str
    ...
    def __call__(self, hidden, obs, embedding, dones, activation):
        if self.add_generator_embedding:
            gen_timestep = obs.env_state.timestep
...
class ActorOnlyPixelsRNN(nn.Module):
    """Actor-only version of ActorCriticPixelsRNN — no critic parameters."""
    action_dim: Sequence[int]
    fc_layer_depth: int
    fc_layer_width: int
    action_mode: str

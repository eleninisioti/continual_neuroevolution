...
class BaseTrainer(eqx.Module):
	...
	end_of_task_eval_fn: Optional[Callable]
	#-------------------------------------------------------------------
	...
	def __init__(self, 
				 ...
				 end_of_task_eval_fn: Optional[Callable]=None):
		
		self.train_steps = train_steps
		self.progress_bar = progress_bar
		self.logger = logger
		self.end_of_task_eval_fn = end_of_task_eval_fn
	...
	def train_gymnax(self, state: TrainState, key: jax.Array, data: Optional[Data]=None, init_env_state: Optional=None)->TrainState:
	...
	def train_(self, state: TrainState, key: jax.Array, data: Optional[Data]=None, init_env_state: Optional=None)->TrainState:
		...
		for phase in range(num_phases):
			# Clear JIT caches from previous phases to free device memory
			if phase > 0:
				jax.clear_caches()
				gc.collect()
			...
			# ── task transition: re-evaluate archive & reset best tracking ──
			if phase > 0:
				n_archive = state.archive.shape[0]
				reeval_batch = 16  # small batches to avoid OOM (16 × eval_reps rollouts at a time)
				print(f"\n  Re-evaluating archive ({n_archive} members, batch={reeval_batch}) on task {phase} ({env_names[phase]})...")
				fit_parts = []
				for a_start in range(0, n_archive, reeval_batch):
					a_batch = state.archive[a_start:a_start + reeval_batch]
					key, batch_key = jr.split(key)
					batch_fit, _, _, _, _ = self.eval(
						a_batch, batch_key, task_params_init, 0,
						current_init_env_state, noise,
						env=current_env, env_params=current_env_params,
						static_env_params=current_static_env_params,
						init_env_state=current_init_env_state
					)
					jax.block_until_ready(batch_fit)
					fit_parts.append(batch_fit)
				archive_raw_fitness = jnp.concatenate(fit_parts, axis=0)
				# Shape fitness (negate for maximisation) to match archive storage format
				archive_shaped_fitness = self.fitness_shaper.apply(
					state.archive, archive_raw_fitness)
				# Re-sort archive by new fitness (ascending = best first)
				sort_idx = jnp.argsort(archive_shaped_fitness)[
					:state.archive.shape[0]]
				new_archive = state.archive[sort_idx]
				new_fitness = archive_shaped_fitness[sort_idx]
				# Best raw fitness on the new task (used to initialise best_fitness
				# so that get_best_fitness_member can keep tracking correctly when
				# maximize=True and gen_counter > 0).
				best_raw = archive_raw_fitness[jnp.argmax(archive_raw_fitness)]
				# Keep population, update fitness, reset best tracking
				state = state.replace(
					archive=new_archive,
					fitness=new_fitness,
					mean=new_archive[0],
					best_fitness=best_raw,
					best_member=new_archive[0],
				)
				print(f"  Archive re-evaluated. Best raw fitness on new task: "
					  f"{float(best_raw):.2f}")
				# Free memory from re-evaluation before the training scan
				del fit_parts, archive_raw_fitness, archive_shaped_fitness, best_raw
				del sort_idx, new_archive, new_fitness
				gc.collect()
		...
		behaviroral_diversity=data["behaviroral_diversity"],
				...
				return [s, k, task_params, should_stop, env_state, None, None]
			...
			# ── end of task: run evaluation and save GIFs ──────────────
			if self.end_of_task_eval_fn is not None:
				# best_member tracks the best individual for the *current* task
				# (reset at each task transition via archive re-evaluation).
				best_member = state.best_member
				self.end_of_task_eval_fn(
					task_idx=phase,
					env_name=env_names[phase],

...
# CraftaxState and CraftaxTask removed - craftax dependency not used
...
class GymnaxTaskWithPerturbation(eqx.Module):
    ...
    def __call__(
        ...
        current_gen: int=0, env_state: Optional=None, noise=None,  env: Optional[PyTree]=None,
    ...
    def rollout(
        ...
        env_state = GymnaxState(env_state=gymnax_state, obs=obs, reward=0.0, done=False, logit=jnp.zeros(self.action_size))
        ...
        def env_step(carry, x):
            ...
            logit, policy_state = policy(state.env_state.obs, state.policy_state, _key,obs_size=obs_size,action_size=action_size)
            ...
            env_state = GymnaxState(env_state=gymnax_state, obs=obs, reward=reward, done=done, logit=logit)	
        ...
        data = {"policy_states": states.policy_state, "obs": states.env_state.obs, "logit": states.env_state.logit}
...
class GymnaxTask(eqx.Module):
	...
	def rollout(
		...
		policy_state, policy_states = policy.initialize(init_policy_key)
		obs, gymnax_state = self.initialize(init_env_key)
		env_state = GymnaxState(env_state=gymnax_state, obs=obs, reward=0.0, done=False, logit=jnp.zeros(self.env.num_actions))
		init_state = State(env_state=env_state, policy_state=policy_state)
		...
		def env_step(carry, x):
			...
			env_state = GymnaxState(env_state=gymnax_state, obs=obs, reward=reward, done=done, logit=jnp.zeros(action_size))	
			new_state = State(env_state=env_state, policy_state=policy_state)
...
class _SingleKinetixTask(eqx.Module):
    ...
    def rollout(
        ...
        init_env_state = GymnaxState(env_state=env_state,  obs=obs,reward=0.0,done=dones, logit=jnp.zeros(20))
        ...
        def env_step(carry, x):
            ...
            new_state = GymnaxState(env_state=env_state, obs=obs,reward=reward,done=done, logit=action)
...
class KinetixTask(eqx.Module):
    ...
    def rollout(
        ...
        # Get action size from environment's action space
        action_size = env.action_space(env_params).shape[0]
        obs_size = 20
        
        init_gymnax_state = GymnaxState(env_state=env_state_reset, obs=obs, reward=0.0, done=dones, logit=jnp.zeros(action_size, dtype=jnp.float32))
        ...
        def env_step(carry, x):
            ...
            new_state = GymnaxState(env_state=env_state_step, obs=obs, reward=reward, done=done, logit=action.astype(jnp.float32))
            new_state = State(env_state=new_state, policy_state=policy_state)

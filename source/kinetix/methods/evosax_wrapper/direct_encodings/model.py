...
def make_model(config, key, env=None, env_params=None, kinetix_config=None, actor_only=False):
    ...
    if config["model_config"]["network_type"] == "MLP":
        ...
        # env is a KinetixTask wrapper; pass the actual kinetix env to make_network_from_config
        actual_env = env.env if hasattr(env, 'env') else env
        network = make_network_from_config(actual_env, env_params, kinetix_config, actor_only=actor_only)
        model = Kinetix(network=network, key=key_model, env=actual_env, env_params=env_params)

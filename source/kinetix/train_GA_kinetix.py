"""
Train GA (SimpleGA) on Kinetix environments (non-continual).
...
def main():
    ...
    sigma_init = args.sigma_init or cfg["sigma_init"]
    ...
    print(f"  Sigma init: {sigma_init}, Elite ratio: {elite_ratio}")
    ...
    config = {
        ...
        'sigma_init': sigma_init, 'elite_ratio': elite_ratio,
    ...
    key, init_key = random.split(key)
    ga_state = ga.init(init_key, jnp.zeros(num_params), ga_params)
    ...
    for gen in range(num_generations):
        ...
        # SimpleGA maximizes by default (unlike OpenES which minimizes)
        ...
        if gen_best > best_overall_fitness:
            ...
            best_idx = np.argmax(fitness_host)
            best_params = jax.device_get(population[best_idx])
    ...
    # Use best individual from final population if no improvement was found

...
from methods.evosax_wrapper.base.tasks.rl import GymnaxTask, GymnaxTaskWithPerturbation, MinatarMultiTask, KinetixTask, BraxTask
...
class EvosaxExperiment(Experiment):
    ...
    def init_model(self):
      self.model = make_model(self.config, self.model_key, env=self.env,
                              ...
                              actor_only=self.config["model_config"].get("actor_only", False))
    ...
    def end_of_task_eval_with_gifs(self, task_idx, env_name, best_member, env, env_params, static_env_params, init_env_state, num_evals=10):
        """Run evaluation rollouts and save GIFs at the end of each task.
        
        best_member: flat params of the best individual from the LAST generation
                     (not the all-time best).
        """
        import numpy as np
        from kinetix.render import make_render_pixels

        print(f"\n  Running {num_evals} eval rollouts for task {task_idx} ({env_name})...")

        # Create output directory for this task's GIFs
        gif_dir = os.path.join(self.config["exp_config"]["trial_dir"], "visuals", "eval", "task_gifs", f"task{task_idx}_{env_name.replace('/', '_')}")
        os.makedirs(gif_dir, exist_ok=True)

        # Reshape best member into policy params
        policy_params = self.params_shaper.reshape_single(best_member)
        policy = eqx.combine(policy_params, self.statics)
        init_policy_state, _ = policy.initialize(jax.random.PRNGKey(0))
        ...
        for eval_idx in range(num_evals):
            rng = jax.random.PRNGKey(seed=eval_idx)
            obs, state = env.reset(rng, env_params=env_params, override_reset_state=init_env_state)
            ...
            for step in range(self.config["env_config"]["episode_length"]):
                ...
                frame = np.array(pixels).astype(np.uint8).transpose(1, 0, 2)[::-1]
                frames.append(frame)
                ...
                obs, state, reward, step_done, _ = jit_env_step(act_rng, state, action, env_params=env_params)
                done = jnp.expand_dims(step_done, axis=0)
            ...
            if frames:
                ...
                gif_path = os.path.join(gif_dir, f"rollout_{eval_idx:02d}_reward{int(cum_reward)}.gif")
                pil_frames[0].save(
                    gif_path,
                    save_all=True,
        ...
        mean_reward = onp.mean(total_rewards)
        print(f"  Task {task_idx} ({env_name}): mean_reward={mean_reward:.2f}, saved {num_evals} GIFs → {gif_dir}")
    ...
    def train_trial(self):
        ...
        # Set up end-of-task evaluation callback for kinetix (generates GIFs)
        if self.config["env_config"]["env_type"] == "kinetix":
            end_of_task_eval_fn = self.end_of_task_eval_with_gifs
        else:
            end_of_task_eval_fn = None
        ...
        trainer = EvosaxTrainer(train_steps=self.config["optimizer_config"]["optimizer_params"]["generations"],
                                ...
                                n_devices=1,
                                eval_reps=2,
                                end_of_task_eval_fn=end_of_task_eval_fn)

...
import sys
import os
# Prefer repo root and local methods directory so local packages shadow pip-installed ones
# `train.py` is at `scripts/train/evosax/`; go three levels up to reach the repo root
...
# Add methods/Kinetix FIRST so `import kinetix` uses local version, not source/kinetix
kinetix_path = os.path.join(ROOT, "methods", "Kinetix")
if kinetix_path not in sys.path:
    sys.path.insert(0, kinetix_path)
# Note: keep CUDA device selection via env var if needed externally
...
def train_kinetix(num_trials, env_name,  optimizer_name, actor_only=False):
...
def train_kinetix_lifelong(num_trials, optimizer, actor_only=False):
    ...
    train_kinetix(num_trials=num_trials, env_name=env_name, optimizer_name=optimizer, actor_only=actor_only)  # Commented out since function doesn't exist
    

def train_kinetix_all(num_trials, optimizer_name, actor_only=False):
    ...
    for env_name in env_names:
        ...
        train_kinetix(num_trials=num_trials, env_name=env_name, optimizer_name=optimizer_name, actor_only=actor_only)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--actor_only", action="store_true", help="Use actor-only network (no critic parameters)")
    ...
    train_kinetix_lifelong(num_trials=args.num_trials, optimizer=args.optimizer, actor_only=args.actor_only)

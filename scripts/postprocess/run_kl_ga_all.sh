#!/bin/bash
# Compute KL divergence for all GA continual gymnax experiments.
# Runs compute_kl_gymnax.py for each env × pop_size × trial.
#
# Usage:
#   bash scripts/postprocess/run_kl_ga_all.sh
...
for env in "${ENVS[@]}"; do
    env_dir="projects/gymnax/ga_continual_pop_sweep/$env"
    if [ ! -d "$env_dir" ]; then
    ...
    for pop_dir in "$env_dir"/pop_*/; do
        ...
        for trial_dir in "$pop_dir"/trial_*/; do
            ...
            if [ -f "$trial_dir/kl_divergence.json" ]; then
                echo "Skipping GA $env $pop $trial (already computed)"
                continue
            fi

            echo ""
            echo ">>> GA | $env | $pop | $trial"

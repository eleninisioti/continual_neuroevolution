"""
Plot continual RL performance vs updates-per-task from the updates sweep.

Reads per-task checkpoints from:
  projects/gymnax/rl_epochs_sweep/{env}/epochs_{N}/trial_{T}/checkpoints/task_{K}.pkl

Creates two plots:
  1. Average reward across all 10 tasks vs updates-per-task (barplot)
  2. Success rate (fraction of tasks solved) vs updates-per-task (barplot)
...
Usage:
    python scripts/postprocess/plot_rl_continual_updates_sweep.py
    python scripts/postprocess/plot_rl_continual_updates_sweep.py --method ppo
    python scripts/postprocess/plot_rl_continual_updates_sweep.py --method ppo trac redo
"""
...
def load_results(sweep_dir, methods):
    ...
    Returns:
        results: {method: {env: {task_interval: [ [task_rewards for trial_1], ... ]}}}
    """
    results = {}
            ...
            for ti in TASK_INTERVALS:
                ...
                for trial in range(1, 100):
                    ckpts_dir = os.path.join(
                        sweep_dir, env, f"epochs_{ti}", f"trial_{trial}", "checkpoints"
                    )
                ...
                if trial_task_rewards:
                    results[method][env][ti] = trial_task_rewards
...
def compute_metrics(results):
    """
    Compute per-task-interval metrics:
    ...
    for method in results:
        ...
        for env in ENVS:
            ...
            for ti in sorted(results[method].get(env, {}).keys()):
                trial_data = results[method][env][ti]
...
def plot_results(metrics, methods, output_dir):
    ...
    for ax, env in zip(axes, ENVS):
        # Collect all task_intervals that have data across any method
        all_tis = sorted(set(
            ti for m in methods for ti in metrics.get(m, {}).get(env, {}).keys()
        ))
        if not all_tis:
            ax.set_title(env)
        ...
        x = np.arange(len(all_tis))

        for mi, method in enumerate(methods):
            means = []
            stds = []
            for ti in all_tis:
                if ti in metrics.get(method, {}).get(env, {}):
                    m = metrics[method][env][ti]
            ...
            for i, ti in enumerate(all_tis):
                if ti in metrics.get(method, {}).get(env, {}):
                    n = metrics[method][env][ti]['num_trials']
                    ax.text(i + offset, means[i] + stds[i] + 0.02 * max(abs(v) for v in means if v != 0),
                            f"n={n}", ha='center', va='bottom', fontsize=7, color='gray')

        ax.set_xticks(x)
        ax.set_xticklabels([str(ti) for ti in all_tis], rotation=45)
        ax.set_xlabel("Updates per Task")
    ...
    fig.suptitle("Continual RL: Average Reward vs Updates per Task", fontsize=14, y=1.02)
    plt.tight_layout()
    path1 = os.path.join(output_dir, "continual_updates_sweep_reward.png")
    ...
    fig.suptitle("Continual RL: Task Success Rate vs Updates per Task", fontsize=14, y=1.02)
    plt.tight_layout()
    path2 = os.path.join(output_dir, "continual_updates_sweep_success.png")
...
def main():
    parser = argparse.ArgumentParser(description="Plot continual RL updates-per-task sweep results")
    parser.add_argument("--sweep_dir", type=str,
                        default="projects/gymnax/rl_epochs_sweep",
    ...
    # Print summary table
    print(f"\n{'Method':<8} {'Env':<16} {'Updates':>8} {'AvgReward':>12} {'Std':>10} {'Success':>10} {'N':>4}")
    print("-" * 70)
    for method in methods:
        for env in ENVS:
            for ti in sorted(metrics.get(method, {}).get(env, {}).keys()):
                m = metrics[method][env][ti]
                print(f"{method:<8} {env:<16} {ti:>8} {m['avg_reward_mean']:>12.2f} "

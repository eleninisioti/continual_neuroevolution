"""
Plot continual GA performance vs population size from the pop-size sweep.
...
Usage:
    python scripts/postprocess/plot_ga_continual_pop_sweep.py
"""
...
def plot_results(metrics, output_dir):
    ...
    fig.suptitle("Continual GA: Average Reward vs Population Size", fontsize=14, y=1.02)
    plt.tight_layout()
    path1 = os.path.join(output_dir, "continual_pop_sweep_reward.png")
    ...
    for ax, env in zip(axes, ENVS):
        ...
        ax.bar(x, means, yerr=stds, capsize=4, color='coral',
               edgecolor='black', alpha=0.85)
        ax.set_xticks(x)
        ax.set_xticklabels([str(p) for p in pop_sizes], rotation=45)
        ax.set_xlabel("Population Size")
    ...
    fig.suptitle("Continual GA: Task Success Rate vs Population Size", fontsize=14, y=1.02)
    plt.tight_layout()
    path2 = os.path.join(output_dir, "continual_pop_sweep_success.png")
...
def main():
    parser = argparse.ArgumentParser(description="Plot continual GA pop-size sweep results")

...
# Non-continual SimpleGA training on 20 medium Kinetix tasks (h0-h19)
# Uses pixel-based observations by default
#
# Usage:
#   ./run_GA_kinetix_noncontinual.sh                  # Run on GPU 0
#   ./run_GA_kinetix_noncontinual.sh --cuda 3         # Run on GPU 3
#   ./run_GA_kinetix_noncontinual.sh --symbolic       # Run with symbolic observations
#   ./run_GA_kinetix_noncontinual.sh --seed 42        # Run with specific seed
...
mkdir -p "$PROJECT_DIR"
mkdir -p "$KINETIX_DIR/data/outputs_ga_noncontinual"
...
for env_name in "${ENVIRONMENTS[@]}"; do
    ...
    for seed in $SEEDS; do
        ...
        CUDA_VISIBLE_DEVICES=$GPU python3 train_GA_kinetix.py \
            --env "$env_name" \
            $SYMBOLIC_FLAG \
            --seed $seed \
            --trial $trial_idx \
            ...
            >> "data/outputs_ga_noncontinual/${env_name}_seed${seed}.txt" 2>&1
        
        echo "    Done (see data/outputs_ga_noncontinual/${env_name}_seed${seed}.txt)"
        ((trial_idx++))
    done
done
...
echo ""
echo "========================================"
echo "All GA training completed!"
echo "========================================"
